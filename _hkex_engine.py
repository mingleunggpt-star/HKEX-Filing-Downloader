...
import asyncio, hashlib, html, json, mimetypes, os, re, tempfile, zipfile
...
BLOB_API_VERSION='12'
HEADERS={'User-Agent':'Mozilla/5.0','Accept':'application/json,text/plain,*/*','Referer':f'{HKEX_BASE}/search/titlesearch.xhtml','Accept-Language':'zh-HK,zh;q=0.9,en;q=0.8'}
...
@dataclass
class Filing:
    record_id:str; publication_date:str; document_type:str; title:str; source_url:str
    is_superseded:bool=False; status:str='PENDING'; size:int=0; sha256:str=''; download_url:str=''; blob_pathname:str=''; error:str=''
...
def safe_name(t:str,n:int=90)->str:
    t=strip_html(t); t=re.sub(r'[<>:"/\\|?*\x00-\x1f]','_',t); t=re.sub(r'\s+',' ',t).strip(' ._'); return (t[:n].rstrip() or 'untitled')
...
def normalize_url(link:str)->str:
    ...
    return HKEX_BASE+(link if link.startswith('/') else '/'+link)
...
async def resolve_stock(client:httpx.AsyncClient,code:str)->tuple[str,str]:
    ...
    r=await client.get(PREFIX_URL,params=p); r.raise_for_status(); data=clean_jsonp(r.text)
    c=[]
    for item in data.get('stockInfo',[]):
        d=re.sub(r'\D','',str(item.get('code','')))
        if d and d.zfill(5)==code:c.append(item)
    if not c: raise RuntimeError(f'Unable to resolve HKEX stock {code}.')
    item=c[0]; stock_id=str(item.get('stockId') or item.get('stockID') or '')
...
async def search_t2(client,stock_id,t2code,from_date,to_date):
    ...
    r=await client.get(SEARCH_URL,params=p); r.raise_for_status(); outer=r.json(); result=outer.get('result',[])
...
def build_filings(code,items):
    ...
    for item in items:
        typ=classify(item)
        if not typ: continue
        url=normalize_url(get_field(item,'FILE_LINK','fileLink','url')); title=strip_html(get_field(item,'TITLE','title'))
        if not url or url in seen:continue
        pub=parse_date(item); rid=hashlib.sha1(f'{code}|{pub}|{url}'.encode()).hexdigest()[:14]; sup=is_superseded(item)
        out.append(Filing(rid,pub,typ,title,url,sup,'SKIPPED_SUPERSEDED' if sup else 'PENDING')); seen.add(url)
...
def normalized_store_id()->str:
    sid=os.environ.get('BLOB_STORE_ID','').strip()
    if not sid: raise RuntimeError('BLOB_STORE_ID is missing. Reconnect the Vercel Blob store to this project.')
    return sid[6:] if sid.startswith('store_') else sid

async def upload_blob_oidc(client:httpx.AsyncClient,local_path:Path,pathname:str,oidc_token:str)->tuple[str,str]:
    headers={'Authorization':f'Bearer {oidc_token}','x-vercel-blob-store-id':normalized_store_id(),'x-api-version':BLOB_API_VERSION,'x-vercel-blob-access':'public','x-content-type':mimetypes.guess_type(local_path.name)[0] or 'application/octet-stream','x-add-random-suffix':'1'}
    url=f"{BLOB_API_URL}?pathname={quote(pathname,safe='/')}"
    with local_path.open('rb') as f:
        r=await client.put(url,headers=headers,content=f.read())
    if not r.is_success:
        try: detail=r.json()
        except Exception: detail=r.text[:1000]
        raise RuntimeError(f'Vercel Blob OIDC upload failed ({r.status_code}): {detail}')
    data=r.json(); dl=data.get('downloadUrl') or data.get('download_url') or data.get('url') or ''; bp=data.get('pathname') or pathname
    if not dl: raise RuntimeError('Vercel Blob upload returned no download URL.')
    return str(dl),str(bp)

async def run_download(stock:str,years:int,*,oidc_token:str):
    code=normalize_code(stock); today=date.today(); from_date=f'{max(1999,today.year-years)}0101'; to_date=today.strftime('%Y%m%d')
    async with httpx.AsyncClient(headers=HEADERS,timeout=httpx.Timeout(90.0,connect=20.0),follow_redirects=True,http2=True) as http:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for filing in filings:
                if filing.is_superseded: continue
                filename=f'{code}_{filing.publication_date}_{safe_name(filing.title,80)}.pdf'; local=raw/filename
                try:
                    await download_pdf(http,filing.source_url,local); filing.size=local.stat().st_size; filing.sha256=sha256_file(local)
                    filing.download_url,filing.blob_pathname=await upload_blob_oidc(http,local,f'HKEX/{code}_{company}/{filing.publication_date}/{filename}',oidc_token); filing.status='DOWNLOADED'
                except Exception as e: filing.status='FAILED'; filing.error=str(e)
            ...
            zip_download_url,zip_blob_pathname=await upload_blob_oidc(http,zip_path,f'HKEX/{code}_{company}/{zip_name}',oidc_token)
        return {'stock_code':code,'company':company,'search_from':from_date,'search_to':to_date,'total_documents':len(filings),'downloaded':sum(f.status=='DOWNLOADED' for f in filings),'failed':sum(f.status=='FAILED' for f in filings),'zip_download_url':zip_download_url,'zip_blob_pathname':zip_blob_pathname,'documents':[asdict(f) for f in filings]}
