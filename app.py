...
app = FastAPI(title="HKEX Filing Downloader v4.2.2")
...
async function fetchPdfWithRetry(doc, maxRetries = 2) {
  let lastError = null;

  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      const proxyUrl =
        `${WORKER_BASE_URL}/?url=${encodeURIComponent(doc.source_url)}&attempt=${attempt + 1}`;
      ...
      const proxyAttempt =
        response.headers.get("x-hkex-proxy-attempt") || String(attempt + 1);
      ...
      if (!contentType.toLowerCase().includes("pdf")) {
        throw new Error(
          `Not PDF: ${contentType || "unknown content-type"}`
          + (upstreamStatus ? ` / HKEX ${upstreamStatus}` : "")
        );
      }
      ...
      if (fileBlob.size < 100) {
        throw new Error(`PDF too small: ${fileBlob.size} bytes`);
      }
...
async function downloadIndexes(indexes) {
  ...
  let savedCount = Number(byId("saved").textContent || "0");
  let failedCount = Number(byId("failed").textContent || "0");
  ...
  for (const index of indexes) {
    ...
    setRowStatus(index, "Downloading…", "downloading");

    try {
      const alreadyExists =
        await fileExists(targetDirectoryHandle, filename);
      ...
      if (alreadyExists && mode === "rename") {
        filename =
          await renamedFilename(targetDirectoryHandle, filename);
      }

      const fileBlob = await fetchPdfWithRetry(doc, 2);

      await saveBlob(targetDirectoryHandle, filename, fileBlob);

      savedCount += 1;
      if (failedIndexes.has(index) && failedCount > 0) {
        failedCount -= 1;
      }
      ...
      const wasAlreadyFailed = failedIndexes.has(index);
      ...
      setRowStatus(
        index,
        `Failed: ${message.slice(0, 110)}`,
        "failed"
      );

      show(byId("warnBox"));
      byId("warnBox").textContent =
        "部分文件下載失敗。可按 Retry Failed 只重試失敗項目；"
        + "失敗列已顯示 proxy / HKEX 診斷資料。";
    }
  ...
  byId("retryBtn").disabled =
    failedIndexes.size === 0 || !targetDirectoryHandle;
}
